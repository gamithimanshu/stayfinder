...
export function Navbar() {
  ...
  const baseLink =
    "relative inline-flex items-center rounded-xl px-4 py-2 text-sm font-medium text-ink-500 transition hover:bg-white hover:text-ink-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-200 focus-visible:ring-offset-2 focus-visible:ring-offset-[#fcfaf4]";
  ...
  const navClass = ({ isActive }) =>
    isActive
      ? cn(
          ...
          "before:content-[''] before:absolute before:left-4 before:top-1/2 before:h-2 before:w-2 before:-translate-y-1/2 before:rounded-xl before:bg-brand-600"
        )
      : baseLink;
    ...
    <header className="sticky top-0 z-50 border-b border-black/5 bg-[#fcfaf4]/85 backdrop-blur-xl">
      <div className="page-shell flex h-[4.5rem] items-center justify-between gap-4">
        <Link to="/" className="flex items-center gap-3">
          <div className="gradient-primary flex h-11 w-11 items-center justify-center rounded-xl text-white shadow-[0_18px_34px_-20px_rgba(15,116,88,0.95)]">
            <Home size={20} />
          </div>
          <div>
        ...
        <nav className="hidden items-center gap-2 rounded-xl border border-black/5 bg-white/70 p-1.5 lg:flex">
          {safeLinks.map((link) => (
        ...
        <div className="hidden items-center gap-3 lg:flex">
          {token ? (
            <>
              <Link to="/wishlist" className="flex h-11 w-11 items-center justify-center rounded-xl border border-black/5 bg-white/80 text-ink-600 transition hover:text-brand-700">
                <Heart size={18} />
              </Link>
              <Link to="/profile" className="flex h-11 w-11 items-center justify-center rounded-xl border border-black/5 bg-white/80 text-ink-600 transition hover:text-brand-700">
                <UserRound size={18} />
              </Link>
        ...
        <button
          type="button"
          className="flex h-11 w-11 items-center justify-center rounded-xl border border-black/5 bg-white/80 text-ink-700 lg:hidden"
